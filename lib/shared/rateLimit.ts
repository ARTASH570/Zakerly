...
// ⚠️ لو إحنا في الإنتاج ومفيش Upstash متظبط، ده مش وضع طبيعي - النسخة البديلة
// في الذاكرة مش دقيقة لو السيرفر شغال بنسخ متعددة بالتوازي (زي Vercel عادةً)،
// فالحدود الفعلية هتبقى أعلى بكتير من المتوقع بصمت. بنسجل تحذير واضح مرة واحدة
// وقت تشغيل السيرفر عشان الموضوع يبان في اللوجات من الإطلاق، مش يتكشف بعدين.
if (process.env.NODE_ENV === 'production' && !hasUpstash) {
  console.error(
    '[rateLimit] تحذير إنتاج: UPSTASH_REDIS_REST_URL/TOKEN مش متظبطين - هنستخدم fallback ' +
      'في الذاكرة، وده مش دقيق لو السيرفر شغال بأكتر من نسخة بالتوازي. اتأكد من ضبط ' +
      'متغيرات Upstash قبل الإطلاق الفعلي.'
  )
}
