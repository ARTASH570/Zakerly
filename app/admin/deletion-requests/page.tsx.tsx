...
export default function AdminDeletionRequestsPage() {
  ...
  async function handleApprove(id: string) {
    if (!confirm('متأكد إنك عايز توافق على حذف الكورس ده؟ هيتمسح نهائيًا مع كل الفيديوهات والبيانات المرتبطة بيه.')) return
    setBusyId(id)
    try {
  ...
  return (
    <DashboardShell navItems={ADMIN_NAV_ITEMS} userName={adminName || 'أدمن'} roleLabel="حساب أدمن">
      <div className="mb-6">
        <h1 className="font-display text-xl font-bold">طلبات حذف الكورسات</h1>
        <p className="text-ink/50 text-sm mt-1">
          الكورسات اللي عليها طلاب مشتركين مش بتتمسح مباشرة - بتيجيلك هنا عشان توافق أو ترفض
        </p>
      </div>
                ...
                <div key={r.id} className="bg-paper border border-ink/10 rounded-xl p-4">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <p className="font-bold text-sm">{r.course_title}</p>
                      <p className="text-ink/50 text-xs mt-0.5">
                        المعلم: {r.teachers?.full_name || '—'}
                      </p>
                      <p className="text-ink/50 text-xs mt-0.5">
                        {r.enrolled_count} طالب مشترك · إجمالي مدفوع {r.total_paid} ج.م
                      </p>
                      {r.reason && (
                        <p className="text-ink/60 text-xs mt-2 bg-ink/5 rounded-lg p-2">
                          سبب المعلم: {r.reason}
                        </p>
                      )}
                    </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {reviewed.map((r) => (
                  ...
                  >
                    <div className="min-w-0">
                      <p className="font-bold text-sm truncate">{r.course_title}</p>
                      <p className="text-ink/40 text-xs mt-0.5">{r.teachers?.full_name || '—'}</p>
                    </div>
                    <span
